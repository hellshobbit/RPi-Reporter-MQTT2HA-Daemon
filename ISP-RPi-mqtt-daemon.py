...
def getLinuxRelease():
    ...
    cmd_locn1 = '/etc/apt/sources.list.d/debian.sources'
    if os.path.exists(cmd_locn1) == True:
      rpi_command =  "/bin/cat /etc/apt/sources.list.d/debian.sources | grep -P 'Suites:' | /usr/bin/awk '{ print $2 }' | /bin/sed -e 's/-/ /g' | /usr/bin/cut -f1 -d' ' | /bin/grep . | /usr/bin/sort -u"
    else:
      rpi_command = "/bin/cat /etc/apt/sources.list | /bin/egrep -v '#' | /usr/bin/awk '{ print $3 }' | /bin/sed -e 's/-/ /g' | /usr/bin/cut -f1 -d' ' | /bin/grep . | /usr/bin/sort -u"

    print_line(rpi_command, debug=True)

    out = subprocess.Popen(rpi_command,
                           shell=True,
                           stdout=subprocess.PIPE,
                           stderr=subprocess.STDOUT)
